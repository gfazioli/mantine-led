...
`Led.Matrix` supports responsive `cols` and `gap` props — just like Mantine's `SimpleGrid`. Pass an object with breakpoint keys to adjust the grid layout at different screen sizes:

<Demo data={demos.matrixResponsive} />
